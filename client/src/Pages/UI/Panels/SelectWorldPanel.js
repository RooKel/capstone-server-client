...
const SelectWorldPanel = (interactable, socket, ftm, navigate, client_data)=>{
    ...
    panel.header.add(new Text({content: 'Select World'}));
    //#endregion
    //#region body
    panel.body.set({
    ...
    const FillTable = ()=>{
        ...
        table.cells.forEach((_, i)=>{
            ...
            MINT.LeftClick(_, ()=>{
                navigate[panel.return_panel.panel].sigs.selected_world.dispatch(list[cur_start_ind + i]);
